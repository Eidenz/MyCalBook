    ...
    return (
        <div className="grid grid-cols-7 flex-1">
            {days.map((day) => {
                ...
                const cellClasses = `border-r border-b border-slate-700 p-2 flex flex-col min-h-[120px] transition-colors duration-300 ${isCurrentMonth ? 'bg-slate-800 hover:bg-slate-700/50' : 'bg-slate-900 text-slate-600'}`;
                const dateClasses = `font-semibold mb-2 self-start ${today ? 'bg-indigo-600 text-white rounded-full w-7 h-7 flex items-center justify-center' : ''} ${!isCurrentMonth ? 'text-slate-700' : ''}`;

                return (
                    <div key={dayKey} className={cellClasses}>
                        <div className={dateClasses}>{format(day, 'd')}</div>
                        <div className="flex-grow overflow-hidden">
                            {visibleEvents.map(event => <EventPill key={event.id} event={event} isStart={isSameDay(day, new Date(event.start_time))} onClick={onEventClick} />)}
                            {hiddenCount > 0 && <div className="text-xs text-slate-400 mt-1 cursor-pointer hover:underline" onClick={() => onShowMoreClick(day, dayEvents)}>+ {hiddenCount} more</div>}
                        </div>
                    </div>
                );
            })}
