        ...
        <div className="p-8 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {eventTypes.map(et => {
                    ...
                    return (
                    <div key={et.id} className="bg-slate-100 dark:bg-slate-800 rounded-lg shadow-lg p-5 flex flex-col border border-slate-300 dark:border-slate-700">
                        <div className="flex justify-between items-start gap-2">
                           <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 break-words">{et.title}</h2>
                           <span className="flex-shrink-0 flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">
                             {et.is_public ? <Eye size={14}/> : <EyeOff size={14}/>}
                             {et.is_public ? 'Public' : 'Private'}
                           </span>
                        </div>
                        <div className="space-y-2 text-slate-400 dark:text-slate-500 dark:text-slate-400 flex-grow">
