    ...
    return (
        <div className="p-8 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                ...
                {selectedSchedule && (
                    <button onClick={handleSaveRules} disabled={isSaving || isLoadingRules} className="px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg font-semibold text-white hover:opacity-90 transition disabled:opacity-50">
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-1/3">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <button onClick={() => handleOpenModal(null)} className="flex items-center gap-1 text-sm text-indigo-400 hover:text-indigo-300"><Plus size={16}/> New</button>
                    </div>
                    {isLoadingSchedules ? <p>Loading...</p> : (
                        <div className="bg-slate-800 rounded-lg p-3 space-y-2">
                            {schedules.map(schedule => (
                                <div key={schedule.id} onClick={() => setSelectedScheduleId(schedule.id)} className={`flex justify-between items-center p-3 rounded-md cursor-pointer transition-colors ${selectedScheduleId === schedule.id ? 'bg-indigo-600/30 text-white' : 'hover:bg-slate-700/50 text-slate-300'}`}>
                                    ...
                                    <div className="flex items-center gap-1">
                                        <button onClick={(e) => {e.stopPropagation(); handleOpenModal(schedule)}} className="p-1 text-slate-400 hover:text-white"><Edit size={16}/></button>
                            ...
                            {schedules.length === 0 && <p className="text-slate-500 text-center p-4">No schedules. Click 'New' to create one.</p>}
                        </div>
                    )}
                </div>
                ...
                <div className="md:w-2/3">
                    {isLoadingRules && <div className="p-8 text-center bg-slate-800 rounded-lg">Loading rules...</div>}
                    ...
                    {!selectedSchedule && !isLoadingSchedules && <div className="p-8 text-center text-slate-500 bg-slate-800 rounded-lg">Select or create a schedule to begin.</div>}
                </div>
            </div>
            <InputModal
                isOpen={isModalOpen}
